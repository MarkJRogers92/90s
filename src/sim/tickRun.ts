 ...
 * (so a status applied now is not shortened by the tick that applied it), the
 * player attack is accepted from the pre-movement position, and the bounded
 * child-event queue drains before the terminal check so death keeps priority
 * over room clear.
 */
